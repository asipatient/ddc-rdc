...
export function ProgramCard({ program }: { program: Program }) {
  ...
  return (
    <article id={program.slug} className="card group flex h-full flex-col p-6 rounded-2xl border border-slate-200 bg-white hover:border-brand-blue/30 shadow-sm hover:shadow-md transition-all">
      ...
      <p className="mt-5 flex-1 text-sm leading-relaxed text-slate-600">{program.description}</p>
         ...
         <div className="mt-5 border-t border-slate-100 pt-5">
           ...
           <ul className="mt-3 space-y-2">
             {program.objectives.map((obj) => (
               <li key={obj} className="text-sm text-slate-600 flex gap-2">
