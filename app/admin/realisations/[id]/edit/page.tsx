...
export default async function EditRealisationPage({ params, searchParams }: Props) {
  const { id } = await params;
  const notices = searchParams ? await searchParams : {};
  const store = await readAdminStore();
  const realisation = store.realisations.find((item) => item.id === id);
  ...
  return (
    <AdminPage>
      <AdminPageHeader title="Modifier la réalisation" description="Mettre à jour la réalisation et sauvegarder les changements dans data/admin-store.json." />
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <StatusBadge status={realisation.status} />
        {realisation.needsReview ? (
        ...
        <div className="ml-auto flex flex-wrap gap-2">
          <form action={setItemStatusAction.bind(null, "realisations", realisation.id, "published")}>
            <button type="submit" className="focus-ring rounded-md bg-brand-green px-4 py-2 text-xs font-bold text-white">
              Publier
            </button>
          </form>
          <form action={setItemStatusAction.bind(null, "realisations", realisation.id, "draft")}>
          ...
          <form action={markContentReviewedAction.bind(null, "realisations", realisation.id)}>
            <button type="submit" className="focus-ring rounded-md border border-slate-200 px-4 py-2 text-xs font-bold text-brand-blue dark:border-slate-700 dark:text-slate-100">
              Marquer comme relu
            </button>
          </form>
          <DeleteItemForm collection="realisations" id={realisation.id} confirmMessage="Êtes-vous sûr de vouloir supprimer cette réalisation ? Cette action est irréversible." />
        </div>
      </div>
      <ContentForm
        action={updateItemAction.bind(null, "realisations", realisation.id)}
        title="Informations de la réalisation"
        defaultValues={{
          ...realisation,
          gallery: (realisation.gallery || []).join("\n"),
          images: (realisation.images || []).join("\n")
        }}
        fields={[
